...
- 放行一些端口或者关闭防火墙
  ...
  ```bash
  $ systemclt disable firewalld.service && systemctl stop firewalld.service
  ```
...
```bash
kubeadm init --apiserver-advertise-address 192.168.19.98 --pod-network-cidr=10.244.0.0/16
...
这里我就是从步骤`拉取flannel`开始重启执行了一遍，运行`kubeadm init`前先执行 `kubeadmin reset`，其他两个node需要重新加入也需要选运行 `kubeadm reset`
